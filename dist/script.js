...
window.addEventListener('DOMContentLoaded', function () {
  // DOMContentLoaded отвечаает за то что наши скрипты начинают  выполняться только тогда когда наша дом структура на странице готова.
  'use strict';

  Object(_modules_modal__WEBPACK_IMPORTED_MODULE_1__["default"])(); // мы вызываем ту переменную которую импортировали

  Object(_modules_tabs__WEBPACK_IMPORTED_MODULE_2__["default"])('.glazing_slider', '.glazing_block', '.glazing_content', 'active'); // у active не ставим точку так как tab[i].classList.add(activeClass) и так знает что это класс
...
var forms = function forms() {
    ...
    return regeneratorRuntime.async(function postData$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            // для запуска fetch нам понадобиться url адрес куда мы будем отправлять этот запрос, а также понадобятся данные data которые будут уходить на сервер. А так же мы говорим что внутри этой функции есть асинхронные операции
            document.querySelector('.status').textContent = message.loading;
            _context.next = 3;
            return regeneratorRuntime.awrap(fetch(url, {
              // в фигурных скобках мы будем настраивать запрос, что будет происходить при этом fetch. В переменную result будет записываться промис который нам вернется от сервера. А так же при помощи await он знает что нашему скрипту необходимо дождаться окончания этого запроса
              method: 'POST',
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      e.preventDefault();
      var statusMessage = document.createElement('div'); // Создаем новый блок div и помещаем его в переменную statusMessage
      ...
      var formData = new FormData(item); // мы помещаем новый конструктор new FormData и во внутрь мы помещаем ту форму из которой мы хотим вытащить все данные

      postData('assets/server.php', formData) // первым передаем файл сервера, а вторым наши данные
      .then(function (res) {
        // так как здесь возвращается промис мы прописываем цепочку, то есть говорим then, нам в этом then попадает какой-то результат который прислал нам сервер причем уже в текстовом формате из за return await res.text(). Говорим что возвращается какой то res
        console.log(res);
        statusMessage.textContent = message.success;
      }).catch(function () {
        return statusMessage.textContent = message.failure;
      }) // таким образом мы обрабатываем ошибку при помощи .catch
      .finally(function () {
...
var modals = function modals() {
  ...
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    // эта функция будет отвечать за показ и скрытие модальных окон при нажатии на определенную ссылку
    var trigger = document.querySelectorAll(triggerSelector),
        modal = document.querySelector(modalSelector),
        close = document.querySelector(closeSelector);
    trigger.forEach(function (item) {
      // метод перебора forEach берем потому что мы получаем массив в const trigger = document.querySelectorAll(triggerSelector)
      item.addEventListener('click', function (e) {
        // так как мы можем нажать на ссылку и чтобы она не открывалась в новом окне нам надо отменить стандартное поведение поэтому аргументом будет e(even)
        if (e.target) {
          // если у нас будет таргет на ссылке то мы отменяем стандартное поведение браузера
          e.preventDefault();
        }
    ...
    modal.addEventListener('click', function (e) {
      if (e.target === modal) {
        // если e.target то есть тот элемент куда я кликнул будет строго равен тому элементу собственно модальное окно
        modal.style.display = 'none'; // так мы скрываем модальное окно на странице
  ...
  function showModalByTime(selector, time) {
    // это функция которая будет показывать модальное окно после определенного времени нахождения на странице
    setTimeout(function () {
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  function showTabContent() {
    ...
    // функция по показу определенного таба табов. Аргумент i будет следить на какой по счету таб нажал пользователь. Мы нолик передаем в функцию showTabContent вместо i и content и tab под индексом ноль станут активными по умолчанию если в вызове функции не указан параметр
  ...
  header.addEventListener('click', function (e) {
    // мы навешиваем обработчик события на область header где находятся все табы
    var target = e.target; // e.target это тот элемент на котором произошло событие, то есть то куда кликнул пользователь.
    ...
    target.parentNode.classList.contains(tabSelector.replace(/\./, '')))) {
      // здесь мы проверяем при помощи parentNode что у tabSelector его родителя мы кликнули        
      tab.forEach(function (item, i) {
        // мы перебираем tab и передаем в item и после этого передаем туда второй аргумент i а это уже номер элемента который мы перебираем
        if (target == item || target.parentNode == item) {
          // если пользователь кликнул на перебираемый item или кликнул на его родителя то,
          hideTabContent();
          showTabContent(i);
        }
      });
